...
void MainProcess(void)
{
	...
	LCD_Print(&blcd, 0, 16 * 0, "TIM PWMI (low)");
...
static void PeriodicProcess(void)
{
	...
	LED_TG(R);
	LCD_Print(&blcd, 0, 16 * 1, "Freq: %7.2f kHz", .001f * SystemCoreClock / (TIM2->CCR1 + 1));
	LCD_Print(&blcd, 0, 16 * 2, "Duty: %.4f", 1.f * (TIM2->CCR2 + 1) / (TIM2->CCR1 + 1));
}
