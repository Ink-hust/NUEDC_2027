...
int main(void)
{
	...
	LCD_Print(&blcd, 0, 16 * 0, "ADC Periodic IRQ");
...
static void SampleProcess(void)
{
	...
	BSP_UART_Transmit_DMA(&buart, "%u, %u, %u, %u\n",
		DL_ADC12_getMemResult(ADC_0_INST, ADC_0_ADCMEM_0),
		DL_ADC12_getMemResult(ADC_0_INST, ADC_0_ADCMEM_1),
		DL_ADC12_getMemResult(ADC_0_INST, ADC_0_ADCMEM_2),
		DL_ADC12_getMemResult(ADC_0_INST, ADC_0_ADCMEM_3));
}
