...
int main(void)
{
	...
	LCD_Print(&blcd, 0, 16 * 0, "UART");
	
	BSP_UART_Transmit_DMA(&buart, "Device: 0x%08X\n", FACTORYREGION->DEVICEID);
	BSP_UART_Transmit_DMA(&buart, "User  : 0x%08X\n", FACTORYREGION->USERID);
	BSP_UART_Transmit_DMA(&buart, "Trace : 0x%08X\n", FACTORYREGION->TRACEID);
